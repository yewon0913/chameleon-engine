...
// ═══ 안대장 5단계 프레임워크 ═══
const ANDAEJANG_FRAMEWORK = `마케팅 5단계 프레임워크 (모든 콘텐츠에 적용!):
1. 시장조사: 타겟 고객 정의 → 그들의 고통/욕구/언어 파악
2. 기획: 핵심 메시지 1개 → "이 콘텐츠로 뭘 얻게 하려는가?"
3. 가격: 가치 앵커링 → "원래 X인데 지금 Y" / 무료 샘플 → 유료 전환
4. 브랜딩: 일관된 톤+비주얼+상징 → 세스 고딘 Purple Cow (튀어야 살아남는다!)
5. 유통: 1소스 → 멀티채널 (OSMU) → 릴스+쓰레드+블로그+쇼츠 동시 배포`;

// ═══ 오길비 카피라이팅 핵심 (borao-skills) ═══
const OGILVY_PRINCIPLES = `오길비 광고 원칙 (모든 카피에 적용!):

【포지셔닝 먼저!】 "이 제품이 뭐고 누구를 위한 건가?" → 이걸 정하기 전에 카피 쓰지 마!
심리적 세그멘테이션 > 인구통계 ("25-45세 여성"보다 "절약형 엄마" 가 낫다)

【약속 = 광고의 영혼!】 기능이 아니라 혜택! 유일하고 경쟁력 있는 1가지 약속!
대부분의 광고는 아무것도 약속하지 않는다 → 그래서 실패한다

【헤드라인 = 돈의 80%!】
- 5배 더 많은 사람이 헤드라인만 읽는다
- 반드시 포함: 브랜드명 + 약속 + 혜택
- 뉴스성 > 유머 / 구체적 > 추상적 / 긴 헤드라인 > 짧은 헤드라인
- 타겟 지목: "사장님들 주목!" "매출 고민인 분?"

【많이 말할수록 많이 판다!】 50단어까지 빠르게 떨어지고, 50~500은 거의 안 떨어짐
관심 있는 구매자에게 긴 카피가 작동한다`;

// ═══ 전환 카피라이팅 규칙 (borao-skills) ═══
const CONVERSION_COPY_RULES = `전환 카피라이팅 5원칙:
1. 명확 > 창의적: 둘 중 하나만 → 명확함!
2. 혜택 > 기능: "4시간→15분" > "자동화 기능"
3. 구체적 > 모호함: "streamline" 금지! 숫자+기간+결과!
4. 고객 언어 > 회사 언어: 리뷰/상담에서 쓰는 말 그대로!
5. 1섹션 = 1주장: 논리적 흐름으로 페이지를 내려가라!

금지어: "혁신적" "최적화" "시너지" "원활한" "세계 최고" → 구체적 수치로 대체!
느낌표(!) → 제거! 자신 없어 보인다`;

// ═══ 숏폼 최적화 규칙 ═══
const SHORTFORM_RULES = `숏폼 최적화 (알고리즘 선호!):
- 최적 길이: 20~30초 (완시청률 극대화!)
- 첫 1~3초: 후킹 필수! (스크롤 멈추기!)
  → 세스 고딘 반전 기법: 예상을 깨는 첫 장면/문장!
  → "이거 틀리면 폐업" > "마케팅 잘하는 법"
- 자막 필수: 음소거 시청 70%+ (자막 없으면 이탈!)
  → 한 줄 15자 이내 / 2줄 이하 / 큰 폰트
- 정보 압축: 1콘텐츠 = 1핵심 메시지 (여러 개 넣지 마!)
- 규격: 9:16 (세로형) / 1080×1920px
- CTA: 마지막 5초 "팔로우하면 매주 이런 꿀팁!"
- 모션그래픽: CTR 2~3배 상승 (텍스트만 < 모션그래픽)
- BGM: 트렌딩 오디오 사용 시 노출 3~5배 상승`;
