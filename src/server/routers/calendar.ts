...
export const calendarRouter = router({
  ...
  aiPlan: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      endDate.setDate(today.getDate() + 30); // 30일간
      ...
      const prompt = `${input.industry} 업종의 소상공인을 위한 향후 30일간 콘텐츠 캘린더를 기획해주세요.

오늘 날짜: ${todayStr}
기획 기간: ${startStr} ~ ${endStr} (오늘 이후 30일간)
타겟: ${input.target}
총 ${input.contentCount}개의 콘텐츠를 기획해주세요.

중요 규칙:
- 모든 date는 반드시 "${startStr}" 이상, "${endStr}" 이하여야 합니다
- 절대 오늘(${todayStr}) 이전 날짜에 배치하지 마세요
- 주말 포함 골고루 배치
- date 형식: "YYYY-MM-DD"
...
[{"date": "${startStr}", "title": "콘텐츠 제목", "contentType": "릴스", "channels": ["인스타", "틱톡"]}]`;
      ...
      try {
        ...
        return { plan: parsed, startDate: startStr, endDate: endStr };
      } catch {
        return { plan: content, startDate: startStr, endDate: endStr };
      }
    }),
